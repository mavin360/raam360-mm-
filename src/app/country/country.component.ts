		...
		if(sessionStorage.getItem('selectedOpt')){
			...
			if(sessionStorage.getItem('selectedOpt')=='Custom'){
				this.Sdate= sessionStorage.getItem('selectedFDate');
				 ...
				 this.custIcon=true;
				 this.dataService.getCountryDashboardByDate(this.countryId,this.Sdate,this.Edate)
				  .subscribe(data => {
					  this.countryDashBoardData = data;
					  this.getValuesInSelectedCurrency(data, this.selectedCurrency);  
					 // document.getElementById('pageTitle').innerHTML = this.countryId;      
				  });
			}else{
				this.getDataByDate();
			}
...
custIcon=false;
 getDataByDate(){
	 ...
	 if(arg=='Yesterday'){
		...
		this.dataService.getCountryDashboardByDate(this.countryId,SD,ED)
		  .subscribe(data => {
			  this.countryDashBoardData = data;
			  this.getValuesInSelectedCurrency(data, this.selectedCurrency);  
			 document.getElementById('pageTitle').innerHTML = this.countryId.toString();      
		  });
  ...
  prepareNetMainData(data, currency){
        ...
        Object.keys(data).forEach(function(key,index) {
            ...
            Object.keys(data[key]).forEach(function(keyInner,indexInner) {
                    ...
                    let sale = parseFloat(data[key][keyInner]['Net Sales'][currency]);
                    datArr.push(sale);
             
                  //if(keyInner == allData.Date) {
                    salesVal = parseFloat(data[key][keyInner]['Net Sales'][currency]);
                 // }
  ...
  getSpeedlineData(countryId) {
    this.dataService.getSpeedlineConnectData('Country', countryId)
      .subscribe(data => {
         ...
         this.speedlineValue = parseFloat(data.Value); 
      });     
  }
